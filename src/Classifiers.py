...
def saveMetrics(metrics_results, outputs_path="DHG_team/outputs"):
...
def saveModel(model, model_id, outputs_path="DHG_team/models"):
...
def saveTokenizer(tokenizer, model_id, outputs_path="DHG_team/models"):
...
def naiveBayes(X_train, Y_train, X_test, Y_test):
    ...
    saveMetrics(metrics_results, outputs_path="DHG_team/outputs")
...
def svmCode(X_train, Y_train, X_test, Y_test):
    ...
    metrics_results = getMetrics(
        Y_train, predicted_labels_train, 
...
def cnn(blogs_train, blogs_test, Y_train, Y_test):
    ...
    # save tokenizer
    saveTokenizer(tokenizer, model_id, outputs_path="DHG_team/models")
    ...
    model.add(Dropout(0.3))  # dropout used for regularization
    ...
    model.compile(optimizer='adam',
                  ...
                  metrics=['accuracy', get_f1])  # compile model
    model.summary()
    ...
    plt.savefig(f'DHG_team/outputs/{model_id}_training_accuracy.png')
    plt.close()
    ...
    plt.xlabel('epoch')
    plt.legend(['train', 'test'], loc='upper left')
    plt.savefig(f'DHG_team/outputs/{model_id}_training_f1_score.png')
    plt.close()

    saveMetrics(metrics_results, outputs_path="DHG_team/outputs")
    saveModel(model, model_id, outputs_path="DHG_team/models")
...
def main():
    ...
    logging.basicConfig(
        filename='DHG_team/outputs/example.log', 
        encoding='utf-8', 
        level=logging.DEBUG,
    ...
    if choice == '0':
        ...
        npzFile = np.load('DHG_team/data/preProcessed.npz')
        text = npzFile['text']
        ...
        text, labels = generateData('data/train.csv')
    else:
